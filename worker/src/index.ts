...
export { RouletteTableDO } from './durable-objects/roulette-table'
...
const roomRegistry = new Map<string, 'blackjack' | 'poker' | 'uno' | 'ultimate-uno' | 'hot-potato' | 'roulette'>()
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === '/api/rooms' && request.method === 'POST') {
      let body: {
        code?: string
        gameType?: 'blackjack' | 'poker' | 'uno' | 'ultimate-uno' | 'hot-potato' | 'roulette'
        hostId?: string
        maxPlayers?: number
      ...
      try {
        const namespace = body.gameType === 'poker' ? env.POKER_TABLE : (body.gameType === 'uno' || body.gameType === 'ultimate-uno') ? env.UNO_TABLE : body.gameType === 'hot-potato' ? env.HOT_POTATO_TABLE : body.gameType === 'roulette' ? env.ROULETTE_TABLE : env.BLACKJACK_TABLE
        const id = namespace.idFromName(body.code)
        const stub = namespace.get(id)

        const defaultMax = body.gameType === 'poker' ? 6 : body.gameType === 'hot-potato' ? 8 : body.gameType === 'roulette' ? 8 : 5
        ...
        const unoExtras =
          (body.gameType === 'uno' || body.gameType === 'ultimate-uno')
            ? {
    ...
    if (roomLookupMatch && request.method === 'GET') {
      ...
      for (const type of ['blackjack', 'poker', 'uno', 'ultimate-uno', 'hot-potato', 'roulette'] as const) {
        const ns = type === 'poker' ? env.POKER_TABLE : (type === 'uno' || type === 'ultimate-uno') ? env.UNO_TABLE : type === 'hot-potato' ? env.HOT_POTATO_TABLE : type === 'roulette' ? env.ROULETTE_TABLE : env.BLACKJACK_TABLE
        ...
        try {
          ...
          if (res.ok) {
            ...
            if (info.active) {
              const resolvedType = (info.gameType === 'ultimate-uno' ? 'ultimate-uno' : type) as typeof type
              roomRegistry.set(code, resolvedType)
              return json(request, env, { code, gameType: resolvedType, freePlay: !!info.freePlay })
            }
          }
        } catch { /* ignore */ }
      }
    ...
    if (freeplayMatch && request.method === 'POST') {
      ...
      const namespace =
        gameType === 'poker' ? env.POKER_TABLE
          : (gameType === 'uno' || gameType === 'ultimate-uno') ? env.UNO_TABLE
            : gameType === 'hot-potato' ? env.HOT_POTATO_TABLE
              : gameType === 'roulette' ? env.ROULETTE_TABLE
                : env.BLACKJACK_TABLE
    ...
    if (wsMatch) {
      ...
      roomRegistry.set(roomCode, gameType as 'blackjack' | 'poker' | 'uno' | 'ultimate-uno' | 'hot-potato' | 'roulette')

      const namespace = gameType === 'poker' ? env.POKER_TABLE : (gameType === 'uno' || gameType === 'ultimate-uno') ? env.UNO_TABLE : gameType === 'hot-potato' ? env.HOT_POTATO_TABLE : gameType === 'roulette' ? env.ROULETTE_TABLE : env.BLACKJACK_TABLE
